...
ASPlugin.register('animation_sliders', {
	...
	onload() {
		...
		function getInterval(event) {
			if ((event.shiftKey || AS_PRESSING.overrides.shift) && (event.ctrlOrCmd || AS_PRESSING.overrides.ctrl)) {
				return 0.1;
			} else if (event.shiftKey || AS_PRESSING.overrides.shift) {
				return 1;
			} else if (event.ctrlOrCmd || AS_PRESSING.overrides.ctrl) {
				return 0.5;
			} else {
				return 1;
			}
		...
		ground_speed_slider = new NumSlider('slider_animation_ground_speed', {
			...
			onBefore() {
				...
				if (AS_SETTINGS.ground_plane.value == false) {
					AS_SETTINGS.ground_plane.set(true);
				}
			},
			onAfter() {
		...
		AS.on('update_keyframe_selection', updateSliders);
	},
	onunload() {
		tween_slider.delete();
		...
		AS.removeListener('update_keyframe_selection', updateSliders);
	}
});
