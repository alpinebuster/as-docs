(async function() {
    ...
    const defaultMarkerArray = AS_MARKER_COLORS.map(e => e.id)
    ...
    ASPlugin.register(id, {
        ...
        oninstall() {
            showAbout(true)
            AS.showQuickMessage("Installed Custom Marker Colors!", 3000)
        },
        onload() {
            addAboutButton()
        ...
        onunload() {
            aboutAction.delete()
            AS.showQuickMessage("Uninstalled Custom Marker Colors", 3000)
            ...
            for (const name of Object.keys(customMarkers)) {
                const index = AS_MARKER_COLORS.indexOf(AS_MARKER_COLORS.find(e => e.name === name))
                AS_MARKER_COLORS.splice(index, 1)
    ...
    function createMarkers() {
        const createMarkersDialog = new AS.Dialog({
            ...
            onConfirm(formData) {
                ...
                if ((FormID && formData.name) && !(defaultMarkerArray.includes(FormID))) {
                    AS.showQuickMessage(`Added marker color "${formData.name}"`, 3000)
    ...
    function editMarkers() {
        ...
        for (const color of AS_MARKER_COLORS) {
                            ...
                            E("div").attr("id", "delete_warning_container").append(
                                ...
                                E("div").attr("id", "delete_warning_buttons").append(
                                    ...
                                    E("button").addClass("danger-button").text("Delete").on("click", e => {
                                        AS.showQuickMessage(`Removed "${color.name}" marker`, 3000)
                                        const index = AS_MARKER_COLORS.indexOf(color)
                                        AS_MARKER_COLORS.splice(index, 1)
                                        Canvas.emptyMaterials.splice(index, 1)
    ...
    function exportMarkerColors() {
        if (localStorage.getItem("customMarkers")) {
            ...
            AS.showQuickMessage("Nothing to export!")
        }
    }
        ...
        const infoBox = new Dialog({
            ...
            lines: [`
                ...
                <div id="content">
                    ...
                    <h4>Worth noting:</h4>
                    <p>- Currently, the only way to get rid of your custom markers altogether is to uninstall the plugin and restart AS.</p>
