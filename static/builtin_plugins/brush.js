...
.mt-tool-settings input[type=range] {
    width: inherit;
}
...
class BrushViewerOption {
	constructor(data) {
		...
		this.node = AS_INTERFACE.createElement('div', {class: 'bvoption'});
...
const BrushViewer = {
	...
	children: null,
	node: AS_INTERFACE.createElement('div', {class: 'brushviewer'}),
	optionsnode: AS_INTERFACE.createElement('div', {class: 'bvoptions'}),
	value: null,
	init() {
		this.children = Object.keys(Brushes).map(id => Brushes[id].brushview);
		this.node.append(
			AS_INTERFACE.createElement('p', {class: 'panel_toolbar_label'}, 'Tool Shape'), 
			this.optionsnode
		);
	...
	async success(data) {			
		if (savedata.brushes) {
			...
			for (const brushId in savedata.brushes) {
				...
				if (isDesktop && !brushData.startsWith('data:image/')) {
		...
		allBrushes.onerror = () => 
			AS.showMessageBox('Failed to load saved brushes!');
	}
});
...
const DynamicsCheckList = {
	...
	checked: [],
	node: AS_INTERFACE.createElement('div', { class: 'checklist' }),

	build() {
		this.structure.forEach(([key, dynamic]) => {
			const id = asuid(6);
			const inode = AS_INTERFACE.createElement('div');
			const checkbox = AS_INTERFACE.createElement('input', {id, type:'checkbox'});
			
			inode.append(
				checkbox, 
				AS_INTERFACE.createElement('label', {for: id, title: dynamic.description}, idToTitle(key))
			);
			if (dynamic.settings) {
				const editButton = AS.getIconNode('edit');
				const form = {}
				for (const settingKey in dynamic.settings) {
					const setting = dynamic.settings[settingKey];
					
					form[settingKey] = {label: idToTitle(settingKey), type: AS.isNewerThan('4.5.0-beta') ? 'range': 'number', value: setting[0]};
...
const showErrorInDialog = message => {
	AS.showQuickMessage(message, 2000);
	throw new Error(message)
}
let brushp;
ASPlugin.register('brush', {
	title: "Brush Plus",
	icon: "fa-brush",
	author: "alpinebuster",
	...
	onload() {
		...
		/* Utils */
		const settingsMap = {
			size: AS_BAR_ITEMS.slider_brush_size,
			opacity: AS_BAR_ITEMS.slider_brush_opacity,
			softness: AS_BAR_ITEMS.slider_brush_softness,
		...
		const customBrushDialog = new Dialog({
			...
			lines: ['<span class="subtle" >AS\'s maximum brush size is 50 meaning images with dimensions greater than 50x50 would lose detail.<br>Black/Transparent -> White/Opaque represents base opacity of the brush.</span>'],
			form_first: true,
			onConfirm({file, name})  {
				...
				methods: {
					...
					showPreservePixelsInfo() {
						AS.showMessageBox({
							...
							message: 'When enabled, overlapping pixels of opacity in the same stroke remain intact; resulting in a flat stroke color.<br/>AS\'s default brush uses this behavior.',
						});
					},
					addCustomBrush() {
					...
					removeCustomBrush() {
						...
						.then(() => {
							...
							AS.showQuickMessage(`Succesfully removed '${selected}'!`, 2000)
						})
						.catch(() => AS.showQuickMessage('Failed to remove brush!', 2000));
					}
				},
				data() {
					return {
		...
		brushp = new Tool('brushplus', {
			...
			keybinding: new Keybinding({key: 'b', shift: true}),
			modes: ['paint'],
			onCanvasClick(data) { Painter.startPaintToolCanvas(data, data.event); },
			onSelect() {
				AS_BAR_ITEMS.brush_shape.set('square');
			...
			brush: {
				...
				floor_coordinates: true, //() => AS_BAR_ITEMS.slider_brush_softness.get() == 0,
