(async function () {
  ...
  ASPlugin.register(id, {
    ...
    icon: "palette",
    author: "alpinebuster",
    description: "Customize the start screen's splash art, by adding your own!",
    tags: ["Splash art", "Start screen", "AS"],
    version: "1.1.2",
    min_version: "1.5.2",
		new_repository_format: true,
    variant: "both",
    onload() {
      if (!AS.flags.after_update) setSplashArt()
      styles = AS.addCSS(`
        #splash_screen > i {
      ...
      setTimeout(() => {
        $("#splash_screen").append(
          E("i").attr("id", "customise-splash-art").addClass("material-icons icon custom-splash").text("settings").on("click", async e => {
            ...
            if (!galleryImages) galleryImages = await fetch("https://api.alpinebuster.top/contents/assets/gallery").then(e => e.json()).then(e => e.map(e => e.download_url)).catch(() => {})
            dialog = new Dialog({
              id,
              title,
              buttons: [],
              component: {
                ...
                methods: {
                  ...
                  async importURL() {
                    ...
                    if (!url) return AS.showQuickMessage("Please enter a URL")
                    try {
                  ...
                  async setCurrentSplashArt(image) {
                    ...
                    AS.showQuickMessage("Applied splash art")
  ...
  async function addSplashArt(image) {
    ...
    const updatePreview = () => preview.css({
      aspectRatio: aspectRatios[parseInt($(settingsDialog.form.form_data.aspectRatio.bar).find(".as-select").attr("value"))].replace(":", " / "),
      imageRendering: imageRenderers[parseInt($(settingsDialog.form.form_data.imageRendering.bar).find(".as-select").attr("value"))].toLowerCase(),
      backgroundSize: backgroundSizes[parseInt($(settingsDialog.form.form_data.backgroundSize.bar).find(".as-select").attr("value"))].toLowerCase(),
      backgroundRepeat: backgroundRepeats[parseInt($(settingsDialog.form.form_data.backgroundRepeat.bar).find(".as-select").attr("value"))].toLowerCase().replace(" ", "-")
    })
    updatePreview()
    const observer = new MutationObserver(updatePreview)
    document.querySelectorAll("dialog#splash_art_settings as-select").forEach(e => observer.observe(e, { attributes: true }))
  }
  ...
  function setBackgroundImage(data) {
    if (!customSplash) customSplash = E("div").attr("id", "custom-splash-art").addClass("custom-splash").appendTo("#splash_screen")
    splashArtStyles = AS.addCSS(`
      #splash_screen {
