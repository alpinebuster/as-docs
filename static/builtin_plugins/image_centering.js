(async function() {
	...
	let button;
	const links = {
        github: "https://github.com/alpinebuster",
		chat: "https://chat.alpinebuster.top/wM4CKTbFVN"
	}
	...
	ASPlugin.register(id, {
		...
		icon,
		author,
		description: "Adds a button and a keybinding that centers the image viewport in an Image format.",
		...
		onload() {
			...
			button = new Action("image_center_button", {
				...
				condition: () => AS_FORMAT?.id == "image",
				click: () => center(),
				keybinding: new Keybinding({key: 'c', shift: false})
			})
		...
		onunload() {
			...
			AS.showQuickMessage("Uninstalled Image Centering plugin", 2000);
		}
	})
	...
	function center() {
		setViewport();
		setZoom();			
		AS.showQuickMessage("Centered viewport!", 1500);
	}
	...
	function setViewport() {
		...
		UVEditor.setZoom(AS_CUR_PROJECT.uv_viewport.zoom);
		Vue.nextTick(() => {
			uv_viewport.scrollLeft = AS_CUR_PROJECT.uv_viewport.offset[0] * UVEditor.vue.inner_width + UVEditor.width/2;
			uv_viewport.scrollTop = AS_CUR_PROJECT.uv_viewport.offset[1] * UVEditor.vue.inner_height + UVEditor.height/2;
		})
	}

	// Resets the zoom of the viewport
	function setZoom(zoom) {
		let max_zoom = Math.round((UVEditor.vue.texture ? UVEditor.vue.texture.height : AS_CUR_PROJECT.texture_width) * 32 / UVEditor.width);
		zoom = Math.clamp(zoom, 0.85, Math.clamp(max_zoom, 16, 64));
		UVEditor.vue.zoom = zoom;
		AS_CUR_PROJECT.uv_viewport.zoom = UVEditor.zoom;
		Vue.nextTick(() => {
		...
		const infoBox = new Dialog({
			...
			lines: [`
				...
				<div id="content">
					...
					<h4>How to use:</h4>
					<p>To use this plugin, go to the brush toolbar (top of the image mode) and click the <b>Center Image Viewport</b> button. There will be a confirmation message displayed on-screen once you center the viewport. You can also use the <b>c</b> keybinding.</p>
					<br>
					<div class="socials">
						<a href="${links["github"]}" class="open-in-browser">
							<i class="fa-brands fa-github" style="color:#1DA1F2"></i>
							<label>By ${author}</label>
						</a>
						<a href="${links["chat"]}" class="open-in-browser">
							<i class="icon fas fa-comment-dots" style="color:#727FFF"></i>
