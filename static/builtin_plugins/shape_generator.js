(function() {
    ...
    ASPlugin.register('shape_generator', {
...
function determineShape(shape, variable, value, thickness, border, axis, origin, border_size, angle) {
    ...
    if (shape === 'hexadecagon')
    {
        ...
        if ((diameter/2+origin[2] > 32 || diameter/2+origin[0] > 32 || thickness/2+origin[1] > 32 ||
             origin[2]-diameter/2 < -16 || origin[0]-diameter/2 < -16 || origin[1]-thickness/2 < -16) && AS_FORMAT.canvas_limit) {
    ...
    AS.showMessage('Generated a '+shape+' with a '+variable+' of '+value+' (Border:'+border+')', 'sidebar');
...
function generateHexadecagonFilled(diameter, side, origin, axis, thickness) {
    ...
    //loading message
    //AS.showMessage('Generating shape... Please wait', 'center')
...
onUninstall = function() {
	//Removing entries
	AS.removeMenuEntry('Generate shape')
};
