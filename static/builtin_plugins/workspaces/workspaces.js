(() => {
  ...
  ASPlugin.register(id, {
    ...
    icon: "icon.png",
    author: "alpinebuster",
    description: "Organize your recent projects into workspaces that you can switch between.",
    tags: ["Files", "Management", "AS"],
    version: "1.0.0",
    min_version: "1.5.2",
    ...
    onload() {
      if (!workspaces.length) workspaces.push({
        name: "Default",
        projects: AS_RECENT_PROJECTS,
        active: true
      })

      AS.on("update_recent_project_thumbnail", recentThumbnails)
      styles = AS.addCSS(`
        #workspace-details {
          position: absolute;
          bottom: 20px;
            ...
            addWorkspace() {
              newDialog = new Dialog({
                ...
                onConfirm(form) {
                  ...
                  if (m) return AS.showQuickMessage(`Unsupported character "${m}"`, 2000)
                  if (workspaces.find(e => e.name === form.name)) return AS.showQuickMessage("Workspace already exists", 2000)
                  workspaces.push({
                    name: form.name,
                    projects: []
                  })
              ...
              editDialog = new Dialog({
                ...
                onConfirm(form) {
                  ...
                  if (form.name && form.name !== name) {
                    const m = form.name.match(/[\\/*?"<>|]/)
                    if (m) return AS.showQuickMessage(`Unsupported character "${m}"`, 2000)
                    if (workspaces.find(e => e.name === form.name)) return AS.showQuickMessage("Workspace name already in use", 2000)
                    const workspace = workspaces.find(e => e.name === name)
    ...
    onunload() {
      AS.removeListener("update_recent_project_thumbnail", recentThumbnails)
      action.delete()
      styles.delete()
      dialog.close()
      newDialog?.close()
  ...
  function switchToWorkspace(name) {
    ...
    old.projects = AS_RECENT_PROJECTS.slice()
    old.active = false
    const active = workspaces.find(e => e.name === name)
    active.active = true
    AS_RECENT_PROJECTS.length = 0
    AS_RECENT_PROJECTS.push(...active.projects)
    localStorage.setItem("workspaces", JSON.stringify(workspaces))
    updateRecentProjects()
  ...
  function recentThumbnails() {
    const recents = AS_RECENT_PROJECTS.slice()
    AS_RECENT_PROJECTS.push(...workspaces.filter(e => !e.active).map(e => e.projects).flat())
    setTimeout(() => {
      AS_RECENT_PROJECTS.length = 0
      AS_RECENT_PROJECTS.push(...recents)
    }, 1)
  }
})()