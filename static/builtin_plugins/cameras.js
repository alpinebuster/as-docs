...
ASPlugin.register('cameras', {
	title: 'Cameras',
	icon: 'videocam',
	author: 'alpinebuster',
	description: 'Adds cameras to AS. Cameras allow you to view the model from different angles, and can be animated to create camera paths.',
	...
	onload() {
		...
		new NodePreviewController(CameraElement, {
			setup(element) {
				...
				AS_CUR_PROJECT.nodes_3d[element.uuid] = mesh;
		...
		let add_action = new Action('add_camera', {
			...
			click() {
				...
				AS.dispatchEvent( 'add_camera', {object: base_camera} )
	

				return base_camera
			}
		})
		AS_INTERFACE.Panels.outliner.menu.addAction(add_action, '3')
		...
		const anim_sign = AS.isNewerThan('4.99') ? 1 : -1;
