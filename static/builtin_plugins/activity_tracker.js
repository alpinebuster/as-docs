(() => {
  ...
  const description = "Track how long you spend using AS and working on each project."
  ...
  ASPlugin.register(id, {
    ...
    author: "alpinebuster",
    description,
    about: "This plugin allows you to keep track of how much time you spend using AS and how long you spend working on each project.\n\nTo view your activity stats, go to `Tools > Activity Tracker`. From here you can see all your stats for both AS and the current project.",
    ...
    onload() {
        ...
        component: {
          ...
          template: `
            <div id="tracker-container">
              ...
              <div v-if="AS_CUR_PROJECT" id="project-tracker">
                <div class="tracker-line"></div>
                <h1>Current Project Activity</h1>
                <p>Models must be saved as a asmodel to preserve per project activity tracking.</p>
                <div class="tracker-row">
                  <div class="tracker-col">
                    <h2>Total</h2>
                    <p>{{ durationString(AS_CUR_PROJECT.${id}) }}</p>
                  </div>
                  <div class="tracker-col">
                    <h2>Current session</h2>
                    <p>{{ durationString(AS_CUR_PROJECT.${id} - AS_CUR_PROJECT.${id}_session) }}</p>
                  </div>
                </div>
              </div>
              <div class="tracker-line"></div>
      ...
      if (AS.isWeb) document.addEventListener("visibilitychange", visibilityChange)
      AS.on("select_project", selectProject)
      selectProject()
      focus()
    },
    onunload() {
      ...
      if (AS.isWeb) document.removeEventListener("visibilitychange", visibilityChange)
      AS.removeListener("select_project", selectProject)
  ...
  function selectProject() {
    setTimeout(() => {
      AS_CUR_PROJECT.activity_tracker_session ||= AS_CUR_PROJECT.activity_tracker
    }, 0)
  }
  ...
  function focus() {
    ...
    interval = setInterval(() => {
      activity.clock += 1
      AS_CUR_PROJECT.activity_tracker += 1
  ...
  function blur() {
    ...
    timeout = setTimeout(() => {
      clearInterval(interval)
      running = false
      notification = AS.showToastNotification({
