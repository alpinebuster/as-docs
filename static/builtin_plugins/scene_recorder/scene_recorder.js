...
let childProcessModule = requireUserPermission ? undefined : require('node:child_process');
...
async function processFFmpeg(vars, options, args) {
  ...
  const file = electron.dialog.showSaveDialogSync({
    ...
    defaultPath: `${AS_CUR_PROJECT.name || "scene"}.${args.extension}`
  })
  if (!file) return
  AS.showQuickMessage("Processing...")
  if (args.preprocess) {
  ...
  AS.showQuickMessage(`Saved as ${file}`)
}

function spawn(exe, args, data = { stdio: "ignore" }) {
  const p = getNodeChildProcess().spawn(exe, args, data)
...
async function checkFFmpeg() {
  ...
  const process = getNodeChildProcess()
      ...
      methods: {
        async select() {
          ...
          try {
            ...
            if (!out.startsWith("ffmpeg")) return AS.showQuickMessage("Invalid FFmpeg file")
          } catch {
            return AS.showQuickMessage("Invalid FFmpeg file")
          }
        ...
        reload() {
          Plugins.all.find(e => e.id === id).reload()
          AS.showQuickMessage(`Reloaded ${name}`)
        },
        close() {
          dialog.close()
        }
...
function getNodeChildProcess() {
  if (childProcessModule == undefined)
    childProcessModule = requireNativeModule('node:child_process', {
