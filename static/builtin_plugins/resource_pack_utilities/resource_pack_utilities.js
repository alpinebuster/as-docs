...
// NOTE: builtin plugin
// const requireUserPermission = AS.isNewerThan('1.5.2');
const requireUserPermission = false;

let nodeCryptoModule = requireUserPermission ? undefined : require('node:child_process');
let nodeZlibModule = requireUserPermission ? undefined : require('node:zlib');
let nodeOSModule = requireUserPermission ? undefined : require('node:os');

function getNodeCrypto() {
  if (nodeCryptoModule == undefined) {
    nodeCryptoModule = requireNativeModule('node:crypto', {
      message: 'This permission is required to communicate with AS.',
    });
  }
...
function getNodeZlib() {
  if (nodeZlibModule == undefined) {
    nodeZlibModule = requireNativeModule('node:zlib', {
      message: 'This permission is required to communicate with AS.',
    });
  }

  return nodeZlibModule;
}
function getNodeOs() {
  if (nodeOSModule == undefined) {
    nodeOSModule = requireNativeModule('node:os', {
      message: 'This permission is required to communicate with AS.',
    });
  }
...
const setupPlugin = () => ASPlugin.register(id, {
  title: name,
  icon: "icon.png",
  author: "alpinebuster",
  description,
  tags: ["Editions", "Resource Packs", "Utilities"],
  version: "1.7.0",
  min_version: "1.5.2",
  new_repository_format: true,
  variant: "desktop",
  website: `https://alpinebuster.top/plugins/${id.replace(/_/g, "-")}/`,
  repository: `https://github.com/alpinebuster/as-plugins/tree/main/${id}`,
  bug_tracker: `https://github.com/alpinebuster/as-plugins/issues?title=[${name}]`,
  creation_date: "2025-07-01",
  has_changelog: true,
  async onload() {
    cacheDir = PathModule.join(electron.app.getPath("userData"), "minecraft_assets_cache")
    ...
    new Setting("ab_minecraft_directory", {
      value: directory,
      category: "defaults",
      type: "click",
      ...
      icon: "folder_open",
      click() {
        const dir = AS.pickDirectory({
          title: "Select your .minecraft folder",
          startpath: AS_SETTINGS.GS.ab_minecraft_directory.value
        })
        if (dir) {
          AS_SETTINGS.ab_minecraft_directory.value = dir
    ...
    const methods = {
      selectFolder(title = "folder", key = "folder") {
        const dir = AS.pickDirectory({
          title: `Select ${title}`,
          startpath: PathModule.join(AS_SETTINGS.ab_minecraft_directory.value, "resourcepacks")
        })
        if (dir) {
          this[key] = dir
        }
    ...
    dialog = new Dialog({
      ...
      onConfirm(r, e) {
        if (Keybinds.extra.confirm.keybinding.isTriggered(e)) return false
      },
      async onBuild() {
      ...
      async onOpen() {
        if (!await exists(AS_SETTINGS.ab_minecraft_directory.value)) {
          new Dialog({
            ...
            onClose() {
              const dir = AS.pickDirectory({
                title: "Select your .minecraft folder",
                startpath: AS_SETTINGS.ab_minecraft_directory.value
              })
              if (dir) {
                AS_SETTINGS.ab_minecraft_directory.value = dir
                Settings.saveLocalStorages()
  ...
  onunload() {
    ...
    action.delete()
    action2.delete()
    Object.keys(utilities).forEach(e => AS_BAR_ITEMS[e].delete())
    styles.delete()
    document.getElementById(`${id}-processing-styles`)?.remove()
  }
})
...
function confirm(title, message) {
  return new Promise(fulfil => AS.showMessageBox({
    title,
    ...
    if (!filePath.endsWith("/") && (filePath.startsWith("assets/") || ["pack.mcmeta", "version.json", "pack.png"].includes(filePath)) && !/\.(class|nbt|mcassetsroot)$/.test(filePath)) {
      ...
      if (q === 0) {
        ...
        Object.defineProperty(parsedZip.files[filePath], "content", {
          ...
          enumerable: true,
          get() {
            const c = getNodeZlib().inflateRawSync(this.compressedContent)
...
async function getVersionData(id) {
  ...
  const vanillaDataPath = PathModule.join(AS_SETTINGS.ab_minecraft_directory.value, "versions", id, id + ".json")
...
async function shaCheck(path, sha) {
  return getNodeCrypto().createHash("sha1").update(await fs.promises.readFile(path)).digest("hex") === sha
}
...
async function getVersionAssetsIndex(id) {
  ...
  const vanillaAssetsIndexPath = PathModule.join(AS_SETTINGS.ab_minecraft_directory.value, "assets", "indexes", version.assets + ".json")
...
async function getVersionJar(id) {
  let jar
  const jarPath = PathModule.join(AS_SETTINGS.ab_minecraft_directory.value, "versions", id, id + ".jar")
...
async function getObject(filePath, version, assetsIndex) {
  ...
  const vanillaObjectPath = PathModule.join(AS_SETTINGS.ab_minecraft_directory.value, "assets", "objects", objectPath)
...
const header = `Generated by the Resource Pack Utilities plugin for AS: https://alpinebuster.top/plugins/${id.replace(/_/g, "-")}/\n\n`
...
const batchExporterFormats = Object.fromEntries(Object.entries(Formats).filter(([id, format]) => format.codec?.compile && format.codec.extension && format.codec.extension !== "asmodel").map(e => [e[0], {
  name: e[1].name,
    ...
    methods: {
      selectFolder(title = "folder") {
        const dir = AS.pickDirectory({
          ...
          startpath: this.folder || PathModule.join(AS_SETTINGS.ab_minecraft_directory.value, "resourcepacks")
        })
        if (dir) {
          this.folder = dir
      ...
      load() {
        AS.import({
          ...
          try {
            ...
            AS.showQuickMessage("Invalid ignore list")
          }
        })
      },
      save() {
        if (!this.ignoreList.length) {
          AS.showQuickMessage("The ignore list is empty")
          return
        }
        AS.export({
          extensions: ["json"],
        ...
        }, () => AS.showQuickMessage("Exported Ignore List"))
      }
    },
    styles: `
      display: flex;
  ...
  outputLog: {
    ...
    methods: {
      ...
      copy() {
        ...
        AS.showQuickMessage("Log copied")
      },
      save() {
        AS.export({
          extensions: ["log"],
  ...
  canvasOutput: {
    ...
    methods: {
      ...
      async copy() {
        ...
        AS.showQuickMessage("Copied to clipboard…")
      },
      save() {
        if (this.type === "GIF") {
          AS.export({
...
const utilities = {
      ...
      methods: {
        async execute() {
          ...
          for (const [file, shortened] of files) {
            ...
            if (data.credit === "Made with AS") delete data.credit
  ...
  packCreator: {
    ...
    component: {
      ...
      created() {
        this.folder = formatPath(PathModule.join(AS_SETTINGS.ab_minecraft_directory.value, "resourcepacks"))
      },
      methods: {
        async execute() {
  ...
  batchExporter: {
    name: "Batch Exporter",
    icon: "move_group",
    tagline: "Export every asmodel file in a folder at the same time.",
    description: "Batch Exporter is a tool that will export every asmodel file within a folder to an output folder using the selected format.",
    component: {
      data: {
        inputFolder: "",
        outputFolder: "",
      ...
      methods: {
        async execute() {
          ...
          const files = await listFiles(this.inputFolder, "asmodel")
          ...
          if (!files.length) {
            this.status.finished = true
            this.status.processing = false
            output.error("No `.asmodel` files present in the selected folder")
            return
          }
      ...
      template: `
        <div v-if="!status.processing && !status.finished">
          <h3>Input Folder:</h3>
          <folder-selector v-model="inputFolder" placeholder="Input Folder">folder containing asmodels</folder-selector>
          <h3>Output Folder:</h3>
          <folder-selector v-model="outputFolder" placeholder="Output Folder">folder to export the asmodels to</folder-selector>
    ...
    component: {
      ...
      methods: {
        ...
        async save() {
          ...
          AS.showQuickMessage("Exported CTM")
        }
      },
      template: `
        <h3>Compact CTM:</h3>
    ...
    component: {
      ...
      methods: {
        saveTemplate() {
          ...
          }, () => AS.showQuickMessage("Saved template…"))
        },
        async execute() {
          if (!this.file) {
            this.gif = null
        ...
        async save() {
          ...
          AS.showQuickMessage("Exported clock")
        }
      },
      styles: `
        #template {
    ...
    component: {
      ...
      methods: {
        ...
        async save() {
          ...
          AS.showQuickMessage("Exported animation")
        }
      },
      styles: `
        #template {
