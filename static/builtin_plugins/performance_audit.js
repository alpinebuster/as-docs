...
ASPlugin.register('performance_audit', {
	title: 'Performance Audit',
	icon: 'network_check',
	author: 'alpinebuster',
	description: 'Find performance issues and bottlenecks in your AS installation.',
	...
	onload() {
		...
		performance_audit_action = new Action({
			...
			category: 'help',
			click: async () => {
				let toast = AS.showToastNotification({
				...
				let selection_time = measurePerformance(() => {
					AS_BAR_ITEMS.invert_selection.click();
				}, 4);
				...
				if (AS_SETTINGS.ambient_occlusion_enabled && AS_SETTINGS.ambient_occlusion_enabled.value) {
