(async function () {
  ...
  const links = {
    website: {
      text: "By alpinebuster",
      link: "https://alpinebuster.top/",
      icon: "language",
      colour: "#33E38E"
    },
    chat: {
      text: "Chat Server",
      link: "https://chat.alpinebuster.top/",
      icon: "fas.fa-comment-dots",
      colour: "#727FFF"
    },
    github: {
      text: "Submit Preview Scenes",
      link: "https://gitea.alpinebuster.top/zzz/as-plugins/",
      icon: "fab.fa-github",
      colour: "#6E40C9"
    }
  }
  ...
  ASPlugin.register(id, {
    ...
    author,
    description,
    tags: ["Preview Scenes", "AS"],
    version: "1.1.4",
    min_version: "1.5.2",
		new_repository_format: true,
    variant: "both",
    creation_date: "2025-9-21",
    await_loading: true,
    onload() {
      styles = AS.addCSS(`
        .preview-scene-links {
      ...
      codec = new Codec("preview_scene_codec", {
        ...
        async export() {
          ...
          compiled.settings = {
            renderSide: AS_CUR_PROJECT.preview_scene_render_side,
            lightSide: AS_CUR_PROJECT.preview_scene_light_side,
            lightColour: AS_CUR_PROJECT.preview_scene_light_colour,
            tintColour: AS_CUR_PROJECT.preview_scene_tint_colour,
            shading: AS_CUR_PROJECT.preview_scene_shading
          }
          let [dialog, settings] = await configurePreviewSceneSettings(compiled, {
            export: true
          })
          settings = await settings
          AS_CUR_PROJECT.preview_scene_render_side = AS_SETTINGS.renderSide
          AS_CUR_PROJECT.preview_scene_light_side = AS_SETTINGS.lightSide
          AS_CUR_PROJECT.preview_scene_light_colour = AS_SETTINGS.lightColour
          AS_CUR_PROJECT.preview_scene_tint_colour = AS_SETTINGS.tintColour
          AS_CUR_PROJECT.preview_scene_shading = AS_SETTINGS.shading
          dialog.close()
          AS.export({
            resource_id: 'model',
            type: scope.name,
            ...
            custom_writer: isDesktop ? (a, b) => scope.write(a, b) : null,
        ...
        compile(options) {
          ...
          const previewScene = {}
          if (checkExport("comment", AS_CUR_PROJECT.credit || AS_SETTINGS.credit.value)) previewScene.credit = AS_CUR_PROJECT.credit || AS_SETTINGS.credit.value
          ...
          if (AS_CUR_PROJECT.texture_width !== 16 || AS_CUR_PROJECT.texture_height !== 16) previewScene.texture_size = [AS_CUR_PROJECT.texture_width, AS_CUR_PROJECT.texture_height]
          if (checkExport("elements", clear_elements.length >= 1)) previewScene.cubes = clear_elements
          if (checkExport("groups", (AS_SETTINGS.export_groups.value && Group.all.length))) {
        ...
        async parse(model) {
          if (!model.cubes) return AS.showMessageBox({
            translateKey: "invalid_model",
            icon: "error"
          })
          this.dispatchEvent("parse", {model})
          if (typeof model.credit === "string") AS_CUR_PROJECT.credit = model.credit
          if (Array.isArray(model.texture_size)) {
            AS_CUR_PROJECT.texture_width = model.texture_size[0]
            AS_CUR_PROJECT.texture_height = model.texture_size[1]
          }
          if (model.settings?.renderSide) AS_CUR_PROJECT.preview_scene_render_side = model.AS_SETTINGS.renderSide
          if (model.settings?.lightSide) AS_CUR_PROJECT.preview_scene_light_side = model.AS_SETTINGS.lightSide
          if (model.settings?.lightColour) AS_CUR_PROJECT.preview_scene_light_colour = model.AS_SETTINGS.lightColour
          if (model.settings?.tintColour) AS_CUR_PROJECT.preview_scene_tint_colour = model.AS_SETTINGS.tintColour
          if (model.settings?.shading) AS_CUR_PROJECT.preview_scene_shading = model.AS_SETTINGS.shading
          if (model.texture) {
      ...
      Language.data["format_category.as"] ??= "AS"
      format = new ModelFormat({
        ...
        format_page: {
          component: {
            methods: {
              newPreviewSceneModel() {
                Formats?.preview_scene_model?.new()
              }
            },
            template: `
              <div class="ab-format-page" style="display:flex;flex-direction:column;height:100%">
                <p class="format_description">${description}</p>
                <p class="format_target"><b>Target</b> : <span>AS</span></p>
                <content>
                  ...
                  <p class="markdown">
                    <ul>
                      ...
                      <li><p>To install directly to AS, go to <strong>File > Export > Install Preview Scene in AS</strong>, and fill in the options.</p></li>
                    </ul>
                  </p>
                ...
                <div class="preview-scene-links">${Object.values(links).map(e => `
                  <a href="${e.link}">
                    ${AS.getIconNode(e.icon, e.colour).outerHTML}
                    <p>${e.text}</p>
                  </a>
                `).join("")}</div>
                <div class="button_bar">
                  <button id="create_new_model_button" style="margin-top:20px;margin-bottom:24px;" @click="newPreviewSceneModel">
      ...
      codec.format = format
      AS_BAR_ITEMS.preview_scene.condition = () => AS_CUR_PROJECT.format !== format
      ...
      exportAction2 = new Action({
        ...
        name: "Install Preview Scene in AS",
        icon: "add_to_photos",
        condition: {
          formats: [format.id]
        },
      ...
      manageAction = new Action({
        ...
        children: [
          ...
          new Action({
            ...
            icon: "file_download",
            async click() {
              const sceneData = await fetch("/builtin_plugins/preview_scene_customiser/assets/scenes/scenes.json").then(e => e.json()).catch(() => {})
              if (!sceneData) return new Dialog({
                id: "download_preview_scene_models_connection_failure_dialog",
                title: name,
                lines: ['<h2>Connection failed</h2><span>Please check your internet connection and make sure that you can access <a href="/builtin_plugins/preview_scene_customiser/assets/scenes/scenes.json">GitHub</a></span>'],
                buttons: ["Okay"]
              }).show()
              ...
              for (const category of sceneData) {
                categories.append(E("div").attr("id", `scene_category_${category.id}`).addClass("hidden").append(
                  ...
                  E("div").addClass("scenes_container").append(...category.scenes.map(e => E("div").addClass("scene").attr("data-scene", e.id).append(
                    E("img").attr("src", `/builtin_plugins/preview_scene_customiser/assets/scenes/${e.id}/scene.webp`),
              ...
              $("dialog#download_preview_scene_dialog #download").on("click", async e => {
                if (!selected) return AS.showQuickMessage("Please select a preview scene")
                const scene = await fetch(`/builtin_plugins/preview_scene_customiser/assets/scenes/${selected.id}/scene.asscene`).then(e => e.json()).catch(() => {})
                if (!scene) return AS.showQuickMessage("Unable to load preview scene")
      ...
      if (currentScene) setTimeout(() => {
        if (AS_CUR_PROJECT.format?.id !== format.id) PreviewScene.scenes[currentScene]?.select()
      ...
      AS.on("select_preview_scene", setCurrentPreviewScene)
      AS.on("unselect_preview_scene", removeCurrentPreviewScene)
    },
    onuninstall() {
      localStorage.removeItem("preview_scene")
    ...
    onunload() {
      AS.removeListener("select_preview_scene", setCurrentPreviewScene)
      AS.removeListener("unselect_preview_scene", removeCurrentPreviewScene)
      format.delete()
      codec.delete()
  ...
  async function configurePreviewSceneSettings(model, args) {
    if (!model.texture) return AS.showMessageBox({
    ...
    $("dialog#preview_scene_settings_dialog #preview_scene_texture").on("click", async e => {
      ...
      try {
        ...
        return AS.showQuickMessage("Unable to load texture")
      }
      if (texture.image.width !== newTexture.image.width || texture.image.height !== newTexture.image.height) return AS.showQuickMessage(`Selected texture does not match required dimentions of ${texture.image.width} ⨉ ${texture.image.height}`, 2000)
      texture = newTexture
      const canvas = document.createElement("canvas")
    ...
    $("dialog#preview_scene_settings_dialog #save").on("click", e => AS.export({
      type: "PNG Texture",
      extensions: ["png"],
      name: args?.name,
    ...
    return [dialog, new Promise(fulfil => {
      $("dialog#preview_scene_settings_dialog #import").on("click", e => {
        fulfil({
          renderSide: parseInt(dialog.form.form_data.renderSide.bar.find("as-select").attr("value")),
          lightSide: parseInt(dialog.form.form_data.lightSide.bar.find("as-select").attr("value")),
  ...
  async function importPreviewScene(model, args) {
    ...
    $("dialog#preview_scene_settings_dialog #import").on("click", async e => {
      ...
      if (!name) return AS.showQuickMessage("Invalid name")
      const id = name.toLowerCase().replace(/\s/g, "_")
      if (PreviewScene.scenes[id]?.id) return AS.showQuickMessage("Preview Scene already exists, please pick a different name", 2000) 
      model.color = AS_SETTINGS.GS.tintColour
      model.shading = AS_SETTINGS.shading
      model.render_side = AS_SETTINGS.renderSide
      scenes.push(id)
      new PreviewModel(id, model)
      const lightColour = tinycolor(AS_SETTINGS.lightColour).toRgb()
      ...
      const scene = new PreviewScene(id, {
        ...
        light_side: AS_SETTINGS.lightSide,
  ...
  function editPreviewScene(scene) {
    ...
    codec.parse(model)
    AS_CUR_PROJECT.name = scene.id
    AS_CUR_PROJECT.preview_scene_render_side = scene.preview_models[0].render_side
    AS_CUR_PROJECT.preview_scene_light_side = scene.light_side
    AS_CUR_PROJECT.preview_scene_light_colour = tinycolor.fromRatio(scene.light_color).toHexString()
    AS_CUR_PROJECT.preview_scene_tint_colour = scene.preview_models[0].color
    AS_CUR_PROJECT.preview_scene_shading = scene.preview_models[0].shading
    AS.setStatusBarText(scene.name)
  }
