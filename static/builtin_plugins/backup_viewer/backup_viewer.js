(() => {
  ...
  ASPlugin.register("backup_viewer", {
    title: "Backup Viewer",
    icon: "icon.png",
    author: "alpinebuster",
    description: "View the project backups from the AS start screen.",
    tags: ["Backups", "Start screen", "AS"],
    version: "2.0.0",
    min_version: "1.5.2",
    ...
    has_changelog: true,
    website: "https://alpinebuster.top/plugins/backup-viewer/",
    repository: "https://github.com/alpinebuster/as-plugins/tree/main/backup-viewer",
    bug_tracker: "https://github.com/alpinebuster/as-plugins/issues/new?title=[Backup Viewer]",
    onload() {
      const backupPath = electron.app.getPath("userData") + osfs + "backups"
      ...
      function getBackups() {
        ...
        for (const file of backupFilePaths) {
          ...
          backups.push({
            id: file,
            path,
            name: file.replace(/backup_\d+\.\d+\.\d+_\d+\.\d+_?/, "").replace(/\.asmodel$/, "") || 'no name',
      ...
      StartScreen.vue.$options.methods.openBackup = backup => AS.read([backup.path], {}, files => loadModelFile(files[0]))
